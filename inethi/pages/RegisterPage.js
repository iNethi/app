...
const RegisterPage = ({ onRegisterSuccess, onLoginSuccess }) => {
    ...
    const handleRegister = async () => {
        ...
        try {
            const response = await axios.post(`${baseURL}${registerEndpoint}`, {
                username: username,
                password: password,
            }, {
                headers: {
            ...
            if (response.status === 201) {
                onRegisterSuccess();
                await handleLogin(username, password, onLoginSuccess, setError, setLoading, navigate);
            ...
            if (error.response) {
                if (error.response.status === 409) {
                    Alert.alert('Error', 'User name is already in use. Please login or try a new user name.');
    ...
    return (
        <View style={styles.container}>
            ...
            />
                    ...
                    <Button title="Register" onPress={handleRegister} disabled={isButtonDisabled} />
            {loading && (
                <Dialog visible={true}>
                    <Dialog.Content>
